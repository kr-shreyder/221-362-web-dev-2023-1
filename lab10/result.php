...
<body>
    <header>
        <div class="logo">
            <img src="./mospolytech-logo-white.png" alt="mospolytech-logo">
        </div>
        <div class="title">
            <h1><a href="/10lab">Лабораторная работа №10</a></h1>
            <h2>Шрейдер Кристина Ивановна</h2>
            <h2>221-362</h2>
        </div>
    </header>
    <main>
        ...
        <div class="text-analysis">
            ...
            <p>
                4. Кол-во знаков препинания:
                <?= mb_strlen(preg_replace("/[^\.,:;!?-]/u", "", $text)) ?>
            </p>
            <p>
                5. Кол-во цифр:
                <?= mb_strlen(preg_replace("/[^0-9]/u", "", $text)) ?>
            </p>
            <p>
                6. Кол-во слов:
                <?php
            ...
            <div class="words-block">
                <table>
                    ...
                    foreach ($unique_symbols as $symbol)
                        $unique_symbols_freq[$symbol] = 0; // устанавливаем начальные значения
                    foreach ($symbols as $symbol)
                        $unique_symbols_freq[$symbol]++; // считаем частоту встречаемости символов
                    foreach ($unique_symbols_freq as $unique_symbol => $frequency) { // выводим результат
                        if ($unique_symbol == " ")
                            $unique_symbol = "'space'";  // пробелы заменяем на 'space'
            ...
            <div class="words-block">
                <table>
                    ...
                    foreach ($unique_words as $unique_word)
                        $unique_words_freq[$unique_word] = 0; // устанавливаем начальные значения
                    foreach ($words as $word)
                        $unique_words_freq[$word]++; // считаем частоту встречаемости слов
